...
public class call_receiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (Objects.equals(intent.getAction(), "android.intent.action.PHONE_STATE")) {
            ...
            call_listener customPhoneListener = new call_listener(context, intent);
...
class call_listener extends PhoneStateListener {
    ...
    call_listener(Context context, Intent intent) {
        super();
    ...
    public void onCallStateChanged(int state, String incomingNumber) {
        if (lastState == TelephonyManager.CALL_STATE_RINGING
            ...
            int slot = Integer.parseInt((String) Objects.requireNonNull(Objects.requireNonNull(intent.getExtras()).get("slot")));
            sendSmgWhenMissedCall(incomingNumber, slot);
        }

        lastState = state;
    }

    private void sendSmgWhenMissedCall(String incomingNumber, int simslot) {
        SharedPreferences sharedPreferences = context.getSharedPreferences("data", MODE_PRIVATE);
        SubscriptionManager manager = SubscriptionManager.from(context);
        String DualSim = "";
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
            if (manager.getActiveSubscriptionInfoCount() == 2) {
                DualSim = "\n" + context.getString(R.string.SIM_card_slot) + simslot;
            }
        }
